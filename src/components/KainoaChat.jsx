...
const KLogo = ({ size = 28 }) => (
  <div
    className="shrink-0 flex items-center justify-center rounded-[10px] bg-gradient-to-br from-[#38bdf8] to-[#2563eb] shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)]"
...
export default function KainoaChat() {
  ...
  const findKainoa = (q) => {
    const ql = q.toLowerCase().trim(); let best=null, s=0;
    for(const a of answers) for(const k of a.keywords||[]){ const kl=k.toLowerCase(); let sc=0;
      if(ql===kl) sc=100; else if(ql.includes(kl)) sc=80; else if(kl.includes(ql)) sc=60;
      if(sc>s){s=sc;best=a;}
    } return s>20?best:null;
  };

  const ddg = async(q)=>{ try{ const p=`https://api.allorigins.win/raw?url=${encodeURIComponent(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(q)}`)}`; const h=await fetch(p).then(r=>r.text()); return [...h.matchAll(/<a class="result__a"[^>]*href="([^"]+)"[^>]*>(.*?)<\/a>/g)].slice(0,5).map(m=>`• [${m[2].replace(/<[^>]+>/g,'').trim()}](${m[1]})`).join('\n')||null;}catch{return null}};
  ...
  const send = async()=>{
    ...
    if(useForum){setMessages(m=>[...m,{role:'bot',text:'Searching forum...'}]);const r=await ddg(`site:forum.theeastpacific.com ${q}`);setMessages(m=>[...m.slice(0,-1),{role:'bot',text:r?`**Forum:**\n\n${r}`:'No results',source:'Forum'}]);return;}
    if(useWeb){setMessages(m=>[...m,{role:'bot',text:'Searching web...'}]);const r=await ddg(q);setMessages(m=>[...m.slice(0,-1),{role:'bot',text:r?`**Web:**\n\n${r}`:'No results',source:'Web'}]);return;}
  ...
  const on="border-slate-700 bg-[#1a1f2b] text-slate-200";
  const off="border-slate-800 bg-[#11151f] text-slate-400 hover:bg-[#171c27]";
    ...
    <div>
      <div className="mb-4 flex items-center gap-2.5">
        ...
        {[{k:'k',v:useKainoa,s:setUseKainoa,l:'Kainoa'},{k:'f',v:useForum,s:setUseForum,l:'Forum'},{k:'w',v:useWeb,s:setUseWeb,l:'Web'}].map(p=>(
          <button key={p.k} onClick={()=>p.s(!p.v)} className={`${pill} ${p.v?on:off}`}>
            <span className={`w-4 h-4 rounded-[4px] border-2 flex items-center justify-center transition ${p.v?'bg-sky-500 border-sky-500':'border-slate-600'}`}>{p.v&&<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="3"><path d="M5 13l4 4 10-10"/></svg>}</span>
            {p.l}
          </button>
        ))}
      </div>
      ...
      <div ref={msgsRef} className="mb-3 space-y-3 max-h-[55vh] overflow-y-auto pr-1">
        {messages.map((m,i)=>(
          <div key={i} className={`flex ${m.role==='user'?'justify-end':''}`}>
            <div className="max-w-[92%] rounded-2xl border border-slate-800/80 bg-[#11151f] px-4 py-3 text-[14px] text-slate-200">{m.source&&<div className="mb-1 text-[10px] uppercase tracking-wide text-sky-400">{m.source}</div>}<div dangerouslySetInnerHTML={{__html:m.text.replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" target="_blank" class="text-sky-400 hover:underline">$1</a>').replace(/\n/g,'<br/>')}}/></div>
          </div>
        ))}
      </div>
