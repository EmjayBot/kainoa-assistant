...
export default function KainoaChat() {
  ...
  const searchForum = async (q) => {
    try {
      const res = await fetch(`https://forum.theeastpacific.com/search.json?q=${encodeURIComponent(q)}`);
      ...
      return topics.map(t => {
        ...
        return `• [${t.title}](${link}) — updated ${date}`;
      }).join('\n');
    } catch {
      return null;
    }
  ...
  return (
    <div className="bg-transparent" style={{ isolation: 'isolate' }}>
        ...
        <div className="grid grid-cols-[auto_auto] items-center gap-2 w-fit">
          {[{key:'forum',active:useForum,toggle:()=>setUseForum(v=>!v),label:'Forum'},{key:'kainoa',active:useKainoa,toggle:()=>setUseKainoa(v=>!v),label:'Kainoa'}].map(p=>(
            <div key={p.key} onClick={p.toggle} style={pillReset} className={`${basePill} ${p.active?pillActive:pillIdle}`}>
              <span className={`w-3.5 h-3.5 rounded-[3px] border flex items-center justify-center ${p.active?'bg-cyan-500 border-cyan-500':'bg-[#0f121a] border-slate-600'}`}>
      ...
      <div className="relative">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Ask about TEP..." className="w-full rounded-2xl border border-slate-800 bg-slate-900/70 px-4 py-3.5 pr-24 text-[14px] text-slate-100 placeholder-slate-500 outline-none" />
