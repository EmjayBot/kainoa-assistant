...
export default function KainoaChat() {
  ...
  // FIXED: Uses Jina AI to bypass CORS and parse DDG reliably
  const ddgSearch = async (query) => {
    try {
      ...
      for (const line of text.split('\n')) {
        ...
        if (m &&!m[2].includes('duckduckgo.com') &&!m[2].includes('jina.ai')) {
  ...
  const send = async () => {
    ...
    if (useForum) {
      ...
      const r = await ddgSearch(`${q} site:forum.theeastpacific.com`);
      setMessages(m => [...m.slice(0,-1), { role: 'bot', text: r? `**Forum Results:**\n\n${r}` : 'No forum results found.', source: 'Forum' }]);
      return;
    }
    if (useWeb) {
      setMessages(m => [...m, { role: 'bot', text: 'Searching web...' }]);
      const r = await ddgSearch(q);
      setMessages(m => [...m.slice(0,-1), { role: 'bot', text: r? `**Web Results:**\n\n${r}` : 'No web results found.', source: 'Web' }]);
      return;
    }
    setMessages(m => [...m, { role: 'bot', text: 'Enable Kainoa, Forum, or Web to search.' }]);
  };
    ...
    <div>
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <div className="relative" ref={aiRef}>
          ...
          {aiOpen && (
            <div className="absolute z-20 mt-1.5 w-[110px] rounded-xl border border-slate-800 bg-[#0c1018] shadow-2xl py-1">
              {models.map(m => (
                <button key={m.id} onClick={() => { setModel(m.id); setAiOpen(false); }} className={`w-full text-left px-3 py-1.5 text-[13px] hover:bg-[#161b25] ${model === m.id? 'text-white' : 'text-slate-400'}`}>{m.label}</button>
              ))}
            </div>
          )}
        </div>
        {[
          { k: 'k', v: useKainoa, s: setUseKainoa, l: 'Kainoa' },
          { k: 'f', v: useForum, s: setUseForum, l: 'Forum' },
          { k: 'w', v: useWeb, s: setUseWeb, l: 'Web' },
        ].map(p => (
          <button key={p.k} onClick={() => p.s(!p.v)} className={`${pill} ${p.v? on : off}`}>
            <span className={`w-4 h-4 rounded-[4px] border-2 flex items-center justify-center transition ${p.v? 'bg-sky-500 border-sky-500' : 'border-slate-600'}`}>
              {p.v && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="3"><path d="M5 13l4 4 10-10"/></svg>}
            </span>
            {p.l}
          </button>
        ))}
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user'? 'justify-end' : ''}`}>
            <div className="max-w-[92%] rounded-2xl border border-slate-800/80 bg-[#11151f] px-4 py-3 text-[14px] text-slate-200">
              {m.source && <div className="mb-1 text-[10px] uppercase tracking-wide text-sky-400">{m.source}</div>}
              <div dangerouslySetInnerHTML={{ __html: m.text.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" class="text-sky-400 hover:underline">$1</a>').replace(/\n/g, '<br/>') }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="relative">
        ...
        <button onClick={send} className="absolute right-1.5 top-1/2 -translate-y-1/2 h-[36px] px-4 rounded-xl bg-sky-600 text-[14px] font-medium text-white hover:bg-sky-500 active:bg-sky-700">Send</button>
      </div>
    </div>
  );
}
