...
export default function KainoaChat() {
  ...
  // bulletproof loader with base path for GitHub Pages
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const results = await Promise.all(
          manifest.map(async (file) => {
            try {
              const url = `${base}data/responses/${file}`;
              const res = await fetch(url);
              if (!res.ok) {
                console.warn(`[Kainoa] skip ${file}`);
                return [];
              }
        ...
        setMessages(m => [...m, { role: 'bot', text: `Load error: ${e.message}` }]);
      }
    };
    load();
  }, [base]);
  ...
  const send = () => {
    ...
    setTimeout(() => {
      if (!answers.length) {
        setMessages(m => [...m, { role: 'bot', text: 'Still loading answers...' }]);
        return;
      }
      const hit = useKainoa && findKainoa(q);
      setMessages(m => [...m, {
        role: 'bot',
        text: hit? hit.answer : "I don't have that yet. Try 'apply'.",
        source: hit? 'Kainoa' : null
      }]);
    }, 50);
  };
