 ...
 .catch(console.error);
  ...
  const searchWeb = async (query) => {
    ...
    for (const url of sources) {
      try {
        ...
        if (isJson && data.results?.length) return data.results.filter(r=>r.url).slice(0,5).map(r=>`• [${r.title}](${r.url})`).join('\n');
        if (!isJson) {
          const out=[]; const re=/\[([^\]]{5,150})\]\((https?:\/\/[^)\s]+)\)/gi; let m;
          while((m=re.exec(data))&&out.length<5) if(!m[2].includes('brave.com/search')) out.push(`• [${m[1].trim()}](${m[2]})`);
  ...
  const searchForum = async (query) => {
    try {
      ...
      const out=[]; const re=/\[([^\]]+)\]\((\/t\/[^)]+)\)/g; let m;
      while((m=re.exec(text))&&out.length<5) out.push(`• [${m[1]}](https://forum.theeastpacific.com${m[2]})`);
        ...
        {messages.map((m,i)=>(
          <div key={i} className={`flex ${m.role==='user'?'justify-end':''}`}>
            <div className="max-w-[92%] rounded-2xl border border-slate-800/80 bg-[#11151f] px-4 py-3 text-[14px] text-slate-200" style={{fontWeight: m.role==='user'?500:400}}>
              {m.source&&<div className="mb-1 text-[10px] uppercase tracking-wide text-sky-400" style={{fontFamily:"'Geom', sans-serif",fontWeight:700,letterSpacing:'0.06em'}}>{m.source}</div>}
