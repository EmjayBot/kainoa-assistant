...
export default function KainoaChat() {
  ...
  // FINAL WORKING SEARCH - DuckDuckGo site search
  const searchForum = async (q) => {
    try {
      const ddgUrl = `https://html.duckduckgo.com/html/?q=site%3Aforum.theeastpacific.com+${encodeURIComponent(q)}`;
      const proxyUrl = `https://r.jina.ai/http://${ddgUrl.replace('https://', '')}`;
      const text = await fetch(proxyUrl).then(r => r.text());

      const matches = [...text.matchAll(/\[([^\]]+)\]\((https:\/\/forum\.theeastpacific\.com\/[^)]+)\)/g)];
      const results = [];
      const seen = new Set();
  ...
  return (
    <div className="bg-transparent" style={{ isolation: 'isolate' }}>
      ...
      <div className="relative">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Ask about TEP..." className="w-full rounded-2xl border border-slate-800 bg-slate-900/70 px-4 py-3.5 pr-24 text-[14px] text-slate-100 placeholder-slate-500 outline-none focus:ring-1 focus:ring-cyan-900/50" />
