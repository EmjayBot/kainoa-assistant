...
export default function KainoaChat() {
  ...
  const [model, setModel] = useState('off'); // 'off' = AI disabled
  ...
  const controlBase = "flex items-center gap-2 rounded-xl border border-slate-700/60 bg-slate-900/70 px-3 h-9 text-sm hover:bg-slate-800/60";

  const Pill = ({checked, onChange, children}) => (
    <label className={`${controlBase} w-full md:w-auto cursor-pointer`}>
    ...
    <div className="bg-transparent">
      ...
      <div className="mb-4">
        <div className="flex flex-col gap-2 md:flex-row md:items-center">
          {/* One pill for AI + model */}
          <div className="relative w-full md:w-auto">
            <div className={`${controlBase} w-full md:w-[180px] !pr-8 !gap-1.5 ${aiEnabled ? 'border-cyan-800/50' : ''}`}>
              <span className={`text-xs ${aiEnabled ? 'text-cyan-400' : 'text-slate-500'}`}>AI:</span>
              <select
                value={model}
            ...
            <div className="pointer-events-none absolute inset-y-0 right-2.5 flex items-center">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-slate-400"><path d="M6 9l6 6 6-6"/></svg>
            </div>
          </div>
      ...
      <div className="relative">
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()}
          ...
          className="w-full rounded-2xl border border-slate-800 bg-slate-900/70 px-4 py-3.5 pr-24"
        />
