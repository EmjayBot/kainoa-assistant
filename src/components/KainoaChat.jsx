...
export default function KainoaChat() {
  ...
  useEffect(() => {
    fetch(`${base}data/responses/index.json`)
     .then(r => r.json())
     .then(m => Promise.all(m.map((f: string) => fetch(`${base}data/responses/${f}`).then(r => r.json()))))
  ...
  const searchForum = async (q: string) => {
    try {
      ...
      return (data.topics || []).slice(0, 5).map((t: any) => ({
        title: t.title,
  ...
  const send = async () => {
    ...
    if (useForum) {
      ...
      setMessages(m => m.filter(msg => (msg as any).id!== searchId));
