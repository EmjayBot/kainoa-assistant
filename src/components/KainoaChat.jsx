...
export default function KainoaChat() {
  ...
  // Compact pills - matches your screenshot
  const pill = "h-7 px-2.5 inline-flex items-center gap-1.5 rounded-lg border border-slate-700/40 bg-[#141722] text-[12px] text-slate-300 hover:bg-[#1a1f2e] transition-colors";

  return (
    <div className="bg-transparent">
      <div className="mb-4 flex flex-wrap items-center gap-2">

        {/* AI - select IS the pill */}
        <select
          value={model}
          onChange={e=>setModel(e.target.value)}
          className={`${pill} appearance-none pr-6 cursor-pointer focus:outline-none focus:ring-1 focus:ring-cyan-900/50 ${model!=='off'? 'border-cyan-800/60 text-cyan-300' : ''}`}
          style={{
        ...
        {/* Forum */}
        <label className={`${pill} cursor-pointer ${useForum? 'border-cyan-900/50' : ''}`}>
          <input
            type="checkbox"
            checked={useForum}
            onChange={e=>setUseForum(e.target.checked)}
        ...
        {/* Kainoa */}
        <label className={`${pill} cursor-pointer ${useKainoa? 'border-cyan-900/50' : ''}`}>
          <input
            type="checkbox"
            checked={useKainoa}
            onChange={e=>setUseKainoa(e.target.checked)}
            className="w-3.5 h-3.5 rounded-[3px] bg-[#0f121a] border-slate-600 text-cyan-500 focus:ring-0 focus:ring-offset-0"
          />
          <span>Kainoa</span>
        </label>
      ...
      <div className="relative">
        ...
        <button
          onClick={send}
          className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl bg-cyan-600 px-5 py-2 text-sm font-medium hover:bg-cyan-500 active:bg-cyan-700 transition-colors"
        >
          Send
        </button>
      </div>
